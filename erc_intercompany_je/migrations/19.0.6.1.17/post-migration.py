...
def migrate(cr, version):
    ...
    if not _is_param_enabled(env, "erc_intercompany_je.autosetup_on_upgrade", True):
        _logger.info(
            "Migration 19.0.6.1.17: intercompany autosetup skipped by backend parameter."
        )
        return
    _logger.info(
        "Migration 19.0.6.1.17: running intercompany autosetup for all active companies."
    )
